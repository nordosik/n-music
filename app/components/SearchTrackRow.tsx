...
// Вспомогательный безопасный парсер артистов
...
// Выделенный компонент с умной проверкой переполнения и тултипом
      ...
      >
        {artists.map((artist, idx) => (
          <span key={idx} className="inline">
            ...
            {/* Запятая с единым плотным пробелом без лишних mr-1.5 */}
            {idx < artists.length - 1 && (
              <span className="text-zinc-500 font-normal tracking-normal">, </span>
            )}
          </span>
        ))}
      </div>
    ...
    >
      ...
      <div className="flex items-center gap-x-3 truncate flex-1 mr-4 min-w-0">
        ...
        <div className="truncate flex-1 min-w-0">
          <h4
            className={`text-xs font-black uppercase tracking-wide truncate transition-colors duration-300 ${isCurrentTrackPlaying ? 'text-white' : 'text-zinc-400 group-hover:text-white'
              }`}
          >
            {track.title}
          </h4>

          {/* АРТИСТЫ С УМНЫМ ТУЛТИПОМ */}
          <div className="flex items-center mt-0.5 text-[9px] font-bold select-none min-w-0">
            <SearchArtistsTooltip
              artists={getArtistsList(track)}
            ...
            {isEcosystem && (
              <>
                ...
                <span className="text-[9px] uppercase font-black tracking-widest text-emerald-500/80 flex-shrink-0">
      ...
      <div
        onClick={(e) => e.stopPropagation()}
        className="flex items-center gap-4 flex-shrink-0 relative z-20"
      >
        {isEcosystem && (
        ...
        <motion.button
          onClick={() => {
            console.log('Лайк из поиска для трека:', track.title);
          }}
        ...
        >
          <Heart size={20} strokeWidth={2.8} className="drop-shadow-[0_0_8px_rgba(255,255,255,0.1)]" />
        </motion.button>
